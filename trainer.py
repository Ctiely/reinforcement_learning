...
from utils.utils import create_list_dirs, sf01
...
class BaseTrainer(object):
    def __init__(self, sess, model, max_to_keep=5, summary_dir="./graphs", checkpoint_dir=None):
        self.sess = sess
        self.model = model
    ...
    def test(self, env, total_timesteps=10000):
        ...
        observation_s = np.zeros(
            ...
            dtype=np.uint8)
        observation_s = self.observation_stack(env.reset(), observation_s)

        for _ in tqdm(range(int(total_timesteps))):
            actions, values, _ = self.model.step_policy.step(observation_s)
            observation, rewards, dones, _ = env.step(actions)
            for n, done in enumerate(dones):
                if done:
                    observation_s[n] *= 0
            observation_s = self.observation_stack(observation, observation_s)
        env.close()

    def observation_stack(self, new_observation, old_observation):
...
class A2CTrainer(BaseTrainer):
    def __init__(self, sess, a2c_model, num_iterations=10e8, learning_rate=7e-4, reward_discount_rate=0.99, lr_decay_method='linear',
        ...
        super().__init__(sess, a2c_model, max_to_keep, summary_dir, checkpoint_dir)
    ...
    def train(self, envs):
        ...
        self.observation_s = np.zeros(
            ...
            dtype=np.uint8)
        self.observation_s = self.observation_stack(envs.reset(), self.observation_s)
    ...
    def __rollout(self):
        ...
        for n in range(self.num_steps):
            # step_policy进行游戏
            actions, values, _ = self.model.step_policy.step(self.observation_s) # 返回最优行动以及行动的价值[envs.num_env,]
            ...
            self.observation_s = self.observation_stack(observations, self.observation_s)
            mb_rewards.append(rewards)
        mb_dones.append(self.dones)

        # Conversion from (time_steps, num_envs) to (num_envs, time_steps)
        mb_obs = np.asarray(mb_obs, dtype=self.observation_s.dtype).swapaxes(1, 0).reshape(train_input_shape)
...
class PPOTrainer(BaseTrainer):
    def __init__(self, sess, ppo_model, num_iterations=10e8, learning_rate=7e-4, noptepochs=4, reward_discount_rate=0.99,
                 nminibatches=4, lam=0.95, lr_decay_method='linear', max_to_keep=5, save_step=20000,
                 summary_dir="./graphs/PPO", checkpoint_dir=None):
        super().__init__(sess, ppo_model, max_to_keep, summary_dir, checkpoint_dir)
        self.num_steps = self.model.num_steps
        self.save_step = save_step
        self.lam = lam
    ...
    def train(self, envs):
        ...
        tstart = time.time()
        loss_list, policy_loss_list, value_loss_list, policy_entropy_list, fps_list = [], [], [], [], []
        start_step = self.global_step_tensor.eval(self.sess)
        ...
        for step in tqdm(range(start_step, self.num_iterations + 1, 1), initial=start_step,
            ...
            self.cur_step = step
            # 使用旧参数收集一次数据
            obs, returns, actions, values, neglogpacs = self.__rollout()

            num_batch = len(obs)
            current_lr = None
            for _ in range(num_batch):
                current_lr = self.learning_rate_decayed.value()
            # 更新多次参数
            inds = np.arange(num_batch)
            nbatch_train = num_batch // self.nminibatches
            for i in range(self.noptepochs):
                np.random.shuffle(inds)
                for start in range(0, num_batch, nbatch_train):
                    end = start + nbatch_train
                    mbinds = inds[start: end]
                    slices = (arr[mbinds] for arr in (obs, returns, actions, values, neglogpacs))
                    prob_ratio, loss, policy_loss, value_loss, policy_entropy = self.__update(current_lr, *slices)
                    # update会更新参数,而数据仍然是旧参数生成的数据,因此导致prob_ratio不一样
                    # print(prob_ratio)
                    nseconds = time.time() - tstart
                    loss_list.append(loss)
                    fps_list.append(int(((step + i) * self.num_steps * self.envs.num_envs) / nseconds))
    ...
    def __rollout(self):
        ...
        for n in range(self.num_steps):
            # step_policy进行游戏
            actions, values, neglogpacs = self.model.step_policy.step(self.observation_s)
            # 返回最优行动,行动的价值以及上一次行动的负对数概率[envs.num_env,]
        ...
        mb_obs = np.asarray(mb_obs, dtype=self.observation_s.dtype)
        mb_rewards = np.asarray(mb_rewards, dtype=np.float32)
        mb_actions = np.asarray(mb_actions)
        mb_values = np.asarray(mb_values, dtype=np.float32)
        mb_neglogpacs = np.asarray(mb_neglogpacs, dtype=np.float32)
        mb_dones = np.asarray(mb_dones, dtype=np.bool)
        last_values = self.model.step_policy.value(self.observation_s)
        ...
        return tuple(map(sf01, (mb_obs, mb_returns, mb_actions, mb_values, mb_neglogpacs)))

    def __update(self, lr, mb_obs, mb_returns, mb_actions, mb_values, mb_neglogpacs):
        # Updates the model per trajectory for using parallel environments. Uses the train_policy.
        # 使用train_policy更新参数
        advantages = mb_returns - mb_values
        ...
        feed_dict = {self.model.train_policy.X_input: mb_obs,
                     self.model.actions: mb_actions,
                     self.model.advantage: advantages,
                     self.model.reward: mb_returns,
                     self.model.learning_rate: lr,
                     self.model.neg_old_log_probs: mb_neglogpacs}
        prob_ratio, loss, policy_loss, value_loss, policy_entropy, _ = self.sess.run(
            [self.model.prob_ratio, self.model.loss, self.model.policy_gradient_loss, self.model.value_function_loss,
             self.model.entropy, self.model.optimize],
            feed_dict=feed_dict)

        return prob_ratio, loss, policy_loss, value_loss, policy_entropy
...
def test_ppo():
    ...
    envs = SubprocVecEnv([make_env(i) for i in range(3)])
    sess = tf.Session()
    ppo_model = PPOModel(sess, CNNPolicy, name="ppo_model", num_envs=envs.num_envs)
    logging.info("Build Model...")
    ppo_model.build(envs.observation_space.shape, envs.action_space.n)
    ppo_trainer = PPOTrainer(sess, ppo_model, num_iterations=100, save_step=100)
    ppo_trainer.train(envs)
    envs = SubprocVecEnv([make_env(0)])
    ppo_trainer.test(envs)
    sess.close()
