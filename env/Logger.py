...
# https://github.com/MG2033/A2C/blob/master/envs/env_summary_logger.py
import tensorflow as tf


class EnvSummaryLogger(object):
    """
    ...
    def add_summary(self, index, step, summaries_dict=None, summaries_merged=None):
        """
        ...
        if summaries_dict is not None:
            ...
            for summary in summary_list:
                self.summary_writer[index].add_summary(summary, step)
            self.summary_writer[index].flush()
        if summaries_merged is not None:
            self.summary_writer[index].add_summary(summaries_merged, step)
            self.summary_writer[index].flush()
